    ...
    <div className="min-h-screen bg-white">
      <div className="w-full overflow-hidden bg-white md:grid md:min-h-screen md:grid-cols-2">
        <aside className="relative flex min-h-[48vh] flex-col justify-between overflow-hidden bg-gradient-to-br from-[#004fc5] via-[#003a94] to-[#002661] p-8 text-white md:min-h-screen md:p-12">
          <div><img src={logo} alt="Settl" className="h-10 w-10 object-contain" /><span className="mt-8 inline-flex rounded-full bg-white/15 px-3 py-1.5 text-xs font-medium text-blue-100">Sri Lanka Freelance Credit</span><h2 className="mt-6 text-3xl font-extrabold leading-tight tracking-tight">Instant Verification.<br /><span className="text-teal-300">Zero Friction.</span></h2><p className="mt-3 text-sm leading-relaxed text-blue-100">We use your NIC and verified email to authenticate your identity in seconds.</p><div className="mt-6 rounded-xl border border-white/10 bg-white/10 p-4"><p className="text-xs font-semibold">Central Bank &amp; PDPA Compliant</p><p className="mt-2 text-xs leading-snug text-blue-200">Sri Lanka Personal Data Protection Act No. 9 of 2022 and 256-bit bank-grade encryption.</p><p className="mt-3 text-xs font-medium text-teal-300"><span className="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-400" />Colombo Live Gateway</p></div></div>
          <div className="mt-8 grid grid-cols-3 gap-2 text-xs"><Step active label="01 Account" /><Step active={stage === "otp"} label="02 Verify" /><Step label="03 Profile" /></div>
        </aside>
...
function Complete({ go }) { return <div className="flex min-h-screen items-center justify-center bg-[#f8fafc] p-6"><div className="w-full max-w-md rounded-3xl border border-slate-100 bg-white p-10 text-center shadow-xl shadow-slate-200/60"><img src={logo} alt="Settl" className="mx-auto h-14 w-14 object-contain" /><div className="mx-auto mt-6 flex h-14 w-14 items-center justify-center rounded-full bg-teal-50 text-2xl font-bold text-teal-600">✓</div><h1 className="mt-5 text-2xl font-extrabold tracking-tight">Identity verified</h1><p className="mt-2 text-sm leading-relaxed text-slate-500">Your verified financial identity is ready. Complete a few profile details next.</p><button onClick={() => go("personal-details")} className="mt-8 w-full rounded-full bg-[#004fc5] py-3.5 text-sm font-bold text-white shadow-lg shadow-blue-500/25 hover:bg-[#003a94]">Continue to profile →</button></div></div>; }
